...
void ChessServer::init_routes()
{
  ...
  m_svr.Post("/login", [](const httplib::Request &req, httplib::Response &res)
    ...
    // pull salt for username
    // add to password, and hash
    // compare to whats stored in db
    // if not equal, return auth error

    // return session id or cookie or w.e.

    res.set_content("Hello " + body["username"].get<std::string>(), "text/plain"); });
}
