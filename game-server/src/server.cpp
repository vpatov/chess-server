...
std::shared_ptr<spdlog::logger> create_logger() {
    ...
    auto logger = std::make_shared<spdlog::logger>(spdlog::logger("multi_sink", {console_sink, file_sink}));
...
void ChessServer::init_routes() {
    ...
    m_svr->Post("/signup", [this](const httplib::Request &req, httplib::Response &res) {
        json body = json::parse(req.body);
    ...
    m_svr->Post("/login", [this](const httplib::Request &req, httplib::Response &res) {
        json body = json::parse(req.body);
